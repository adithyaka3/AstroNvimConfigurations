...
function M.nav(opts)
    ...
    return M.builder(status_utils.setup_providers(opts, { "ruler" }))
end
    ...
    opts = extend_tbl({
        macro_recording = {
            ...
            update = {
                ...
                callback = vim.fn.has "nvim-0.9" == 0 and vim.schedule_wrap(function() vim.cmd.redrawstatus() end)
                    or nil,
            },
        },
        search_count = {
...
function M.mode(opts)
    opts = extend_tbl({
        mode_text = { str = " %1", hl = "ModeIcon", padding = { right = 1, left = 1 } },
        paste = false,
        spell = false,
